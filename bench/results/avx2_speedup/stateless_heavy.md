...
|------|-----:|------:|-------|
| Scalar (K calls/tick) | 2.61176e+08 | 2.36865e+08 | one call per lane through the existing scalar JIT |
| **Vectorized (1 call/tick, K lanes)** | **3.25198e+08** | 2.57709e+08 | one call processing K MarketStates via `<K x double>` IR |
| Speedup (best/best) | **1.24513×** | | |
...
* `scalar_sink = -2.51535e+10`
* `vec_sink    = -2.51535e+10`
* match: **yes**
...
```
cd build-wsl
./cross_symbol_benchmark ../examples/stateless_heavy.sig 2000000 --lanes=4 --runs=15
```
