...
int main(int argc, char** argv) {
  try {
    if (argc < 2) {
      std::cerr << "Usage: signal_benchmark [--all-signals] <signal_file> [events] [csv_out] [signal_name]\n";
      return 1;
    }
    ...
    jitse::MarketSimulator sim(42, std::max<std::size_t>(1, tickers.size()));
...

    {
      ...
      jitse::MarketSimulator warmup_sim(99, std::max<std::size_t>(1, tickers.size()));
    ...
    if (all_signals_mode ? program_jit.IsAvailable() : backend->IsAvailable()) {
      if (all_signals_mode ? (program_jit.CompileProgram(signals, symbols) && (program_fn = program_jit.GetProgramFunction()) != nullptr)
                           : (backend->Compile(*signal, symbols) && backend->GetFunction() != nullptr)) {
        jit_mode = "enabled";
        ...
        std::vector<double> jit_outputs;
        if (all_signals_mode) {
          jit_outputs.assign(signals.size(), 0.0);
        }
        ...
        jitse::MarketSimulator jit_sim(42, std::max<std::size_t>(1, tickers.size()));

        {
          ...
          jitse::MarketSimulator jit_warmup_sim(99, std::max<std::size_t>(1, tickers.size()));
          ...
          for (std::size_t i = 0; i < kWarmupIters; ++i) {
            ...
            if (all_signals_mode) {
              ...
              jit_warmup_sink += jit_outputs.back();
            } else {
        ...
        jit_error = all_signals_mode ? program_jit.LastError() : backend->LastError();
      }
    } else {
      jit_error = all_signals_mode ? program_jit.LastError() : backend->LastError();
    }
    ...
    if (tickers.size() >= 2) {
      ...
      jitse::MarketSimulator hw_sim(42, tickers.size());
      {
      ...
      for (std::size_t i = 0; i < events; i += kBatch) {
        ...
        for (std::size_t j = 0; j < batch_count; ++j) {
          const auto ev = hw_sim.NextEvent(1000);
